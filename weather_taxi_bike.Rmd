...
weather_transport_df_wv <-
  weather_transport_df %>% 
  drop_na(awnd, velocity) %>% 
  group_by(type, awnd) %>% 
  summarize(v_mean = mean(velocity))

# Draw plot with velocity and precipitation. 
  ggplot(weather_transport_df_wv, aes(x = awnd, y = v_mean, group = type, colour = type)) + 
...
```{r precipitation with velocity and total number of specific transport}
# Drawing plot
...
```{r transportation trends among months}
...
The connection between transport choice, popularity, velocity and the temperature are showing similar trends, considering both the highest and lowest temperature within a day. It could be concluded that people are more willing to travel outside under the moderate temperature, the higher, lower or even extreme temperature the weather is, the less potential likelihood that people will go outside and use the transport. It also comes an interesting phenomenon that taxi tends to run faster and bike runs slower with the temperature increase. The faster of the taxi could because of the decrease of the traffic pressure and this effect is more obvious compared to snow effect while riders tend to drive slower for getting warmer and slower wind speed. 
...
there is no residential housing for the general public on the island.
