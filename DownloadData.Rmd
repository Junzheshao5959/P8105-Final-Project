...
for (i in year) {
  for (j in month) {
    ...
    month_temp <- as.character(j)
    if (j < 10) {
      url <- glue("https://s3.amazonaws.com/tripdata/JC-{year_temp}0{month_temp}-citibike-tripdata.csv.zip")
    }
    else {
      url <- glue("https://s3.amazonaws.com/tripdata/JC-{year_temp}{month_temp}-citibike-tripdata.csv.zip")
    }
    print(url)
    if (url.exists(url)) {
      ...
      if (j < 10) {
        csv_name <- glue("JC-{year_temp}0{month_temp}-citibike-tripdata.csv")
      }
      else {
        csv_name <- glue("JC-{year_temp}{month_temp}-citibike-tripdata.csv")
      }
      unzip(temp, csv_name)
