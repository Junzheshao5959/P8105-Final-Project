    ...
    fluidRow(
      column(width = 9,
       ...
       column(width = 3,
        ...
        selectInput("pick_up", "Pick up location", 
                       choices = as.list(manhattan_sub$zone)), 
        selectInput("drop_off", "Drop off location", 
                    choices = as.list(manhattan_sub$zone)), 
...
server <- function(input, output, session) {
  ...
  pick_up_choose <- reactiveValues()
  pick_up_choose <- NULL
  
  drop_off_choose <- reactiveValues()
  drop_off_choose <- NULL

  observeEvent(input$button, {
    date_choose <- input$date 
    pick_up_choose <- input$pick_up
    drop_off_choose <- input$drop_off
    print(date_choose)
    print(pick_up_choose)
    print(drop_off_choose)
  })
    ...
    #appoint "to"
      ...
      leafletProxy("manhattan_map") %>% 
        addPolygons(data = map_data$choice, 
                    ...
                    weight = 1, 
                    layerId = "to") %>%
        addPolylines(lat = c(coordinate_from$lat,coordinate_to$lat), lng = c(coordinate_from$lng,coordinate_to$lng), layerId = "line", color = "navy", weight = 2, opacity = 0.5)
