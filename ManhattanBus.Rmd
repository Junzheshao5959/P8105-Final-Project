...
```{r filter data}
#filter data
bus_data_filter <- bus_data %>%
  filter(endsWith(arrival_time, ':00')) %>%
  filter(endsWith(departure_time, ':00'))
  
bus_coordinate_data <- inner_join(bus_data_filter, stop_data, by = "stop_id")
```
...
```{r reselect data}
#reselect data
bus_neighborhood <- bus_neighborhood %>%
